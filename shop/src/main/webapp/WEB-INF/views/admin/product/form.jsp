               ...
               <div class="col-6">
                  <div class="form-group mb-3">
                     ...
                     <select class="form-select" name="categoryNo">
                        <option value="xxx">먼저 상위 카테고리를 선택하세요</option>
                     </select>
                  </div>
               </div>
            </div>
...
<script type="text/javascript">
	function changeCategory() {
		...
		let xhr = new XMLHttpRequest(); // AJAX 요청 객체 생성
			...
			if (xhr.readyState == 4 && xhr.status == 200) {
				...
				for (let index = 0; index < categories.length; index++) {
					let cat = categories[index];
												// \${} : JSP의 el표현식이 아님을 표시(자바스크립트에서 사용하는 el표현식)
					html += `<option value="\${cat.no}">\${cat.name}</option>`;
				}
				subSelect.innerHTML = html; // textContent는 태그 사이에 텍스트를 표현한다면, innerHTML은 HTML 자체를 표현
			}
		}
